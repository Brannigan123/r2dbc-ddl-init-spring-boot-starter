...
public class R2dbcSchemaInitializer implements ApplicationRunner {
    ...
    private record ColumnDefaultStrategy(
            boolean isIdentity,
            String defaultValue) {
    }
    ...
    private void createTableIfNotExists(RelationalPersistentEntity<?> entity, String tableName) {
        ...
        for (PhysicalProperty physProp : properties) {
            ...
            ColumnDefaultStrategy defaultStrategy = getColumnDefaultStrategy(property, physProp.isPrimaryKey(), isCompositeKey);

            if (defaultStrategy.isIdentity()) {
                sql.append(" GENERATED BY DEFAULT AS IDENTITY");
            } else if (defaultStrategy.defaultValue() != null) {
                sql.append(" DEFAULT ").append(defaultStrategy.defaultValue());
            }
    ...
    private void synchronizeTableColumns(RelationalPersistentEntity<?> entity, String tableName) {
        ...
        SearchColumnSpec searchSpec = buildSearchColumnSpec(entity, tableName);
        List<PhysicalProperty> properties = getPhysicalProperties(entity);
        long pkCount = properties.stream().filter(PhysicalProperty::isPrimaryKey).count();
        ...
        for (PhysicalProperty physProp : properties) {
            ...
            if (!existingColumns.containsKey(columnName)) {
                ...
                String targetDefault = defaultStrategy.defaultValue();
    ...
    private ColumnDefaultStrategy getColumnDefaultStrategy(RelationalPersistentProperty property, boolean isPrimaryKey, boolean isCompositeKey) {
        Field field = property.getField();
        if (field != null && field.isAnnotationPresent(ColumnDefault.class)) {
            return new ColumnDefaultStrategy(false, field.getAnnotation(ColumnDefault.class).value());
        }
        if (isPrimaryKey && !isCompositeKey) {
            ...
            if (isAutoIncrementType(type)) {
                ...
                return new ColumnDefaultStrategy(false, "gen_random_uuid()");
            }
        }
        return new ColumnDefaultStrategy(false, null);
    }
    ...
    private boolean isTypeMismatched(String targetSqlType, String dbDataType, String dbUdtName) {
        ...
        if (normalizedTarget.equals("smallint")) {
            return !normalizedDbDataType.equals("smallint");
        }
